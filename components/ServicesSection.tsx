      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, idx) => (
            <ServiceCard key={idx} {...service} index={idx} isInView={isInView} />
          ))}
        </div>
      </div>
    </section>
...
function ServiceCard({
  ...
  return (
    <motion.div
      ...
      className="group p-8 bg-white/5 border border-amber-500/30 rounded-xl hover:bg-white/10 hover:border-amber-500/60 transition-all duration-500"
    >
      <motion.div
